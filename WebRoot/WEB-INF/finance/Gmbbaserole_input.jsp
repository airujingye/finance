...
<div class="container-fluid gmp GMAdmin">
	...
	<form style="width:100%;height:100%;display:inline-block;" id="listForm" method="post" class="form-horizontal column ui-sortable gmbbp_cont_treeborderNo" action="Gmbbaserole!${isAddAction?'gmbAdd':'gmbUpdate'}.action" role="form">
			...
			<div class="col-md-10 gmbbp_contentbox gmbbp_height gmbbp_size_right">
				<div class="col-md-12 gmbbp_content_right">
					<div class="col-md-12 gmbbp_con_righttitle">
						<!--内容右侧title部分开始-->
						<div class="gmbbp_conright_logo">&nbsp;&nbsp;${isAddAction?'创建':'编辑'}基础角色</div>
					</div>
					<!--右侧编辑部分-->
					<div class="col-md-12 gmbbp_int_formcreat gmb_pageinfo">
						<div class="col-md-12 gmbbp_int_formlisttitle">
							<ul class="nav nav-tabs gmbbp_tab_panel column ui-sortable" role="tablist">
								<li role="presentation" class="lyrow ui-draggable active" data-setting = "1"><a href="#tabgmbbaserole" aria-controls="home" role="tab" data-toggle="tab">基础角色基本信息</a></li>
													<li role="presentation" class="lyrow ui-draggable" data-setting = "1"><a href="#tab_gmbbaserole_gmbbasemenus" data-toggle="tab">菜单表</a></li>
							</ul>
						</div>
						<div class="col-md-12 tab-content gmbbp_formlistcon">
								...
								<div class="col-md-12 column ui-sortable gmbbp_cont_treeborderNo">
											...
											<div class="col-lg-8 col-md-8 col-sm-8 col-xs-8">
												<input type="text" class="form-control gmbbp_input " maxlength="64"
													name="gmbbaserole.name" value="${gmbbaserole.name}"
													placeholder="角色名称"/>
											</div>
										</div>
								</div>
							...
							<!-- 遍历关联表开始  -->
												<div class="tab-pane" id="tab_gmbbaserole_gmbbasemenus">
														<div class="zTreeDemoBackground left bg-ztree">
															<ul id="treegmbbasemenu" class="ztree GMZtree" gmTreeShowName="name" gmTreeIsAsync="false" gmonasyncsuccess="panelAsyncSuccess" gmTreeUrl="<%=gmBase%>/finance/Gmbbasemenu!gmbList.action?handle=forwardAllList"
																gmTreeActiveData="<c:forEach var="item" varStatus="status" items="${gmbbaserole.gmbbasemenus}">${!status.first?',':''}${item.id}</c:forEach>"></ul>
														</div>
												</div>
									<!-- 遍历关联表结束 -->
								</div>
...
<script type="text/javascript">
	$().ready( function() {
		...
		$(".sub").click(function(){
			...
			/*自动封装菜单表表单*/
			GM.Input.createInputByProperty("gmbbaserole.gmbbasemenus","id","hidden",gmbbasemenus,form[0]);
		});
	});
	/*初始化GreatMap控件*/
	GM.init();
