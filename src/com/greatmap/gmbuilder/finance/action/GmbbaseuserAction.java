 ...
 *
 */
public class GmbbaseuserAction extends BaseAction {
	 ...
	 * 基础用户类
	 */
	protected Gmbbaseuser gmbbaseuser;
	 ...
	 * 基础用户List
	 */
	private List<Gmbbaseuser> gmbbaseuserList;
	...
	private String getAllList() {
		...
		//定义基础用户对象List
		List<Gmbbaseuser> gmbbaseuser = null;
		try {
			//获取所有基础用户
			gmbbaseuser = gmbbaseuserService.getAll(fields);
		} catch (Exception e) {
			e.printStackTrace();
		}
		//返回基础用户列表
		return gmbbaseuser!=null?WriteJsonArray(JSONArray.fromObject(gmbbaseuser)):NONE;
	}
	 ...
	 */
	 private Page getList(){
		 ...
		 if (fullSearch != null && fullSearch.trim().length() > 0) {
			try {
				page = gmbbaseuserService.findFullByPage(page,super.searchColumns,fullSearch);
			} catch (Exception e) {
				e.printStackTrace();
			}
		 }else{
			...
			try {
				page = gmbbaseuserService.findByPager(page,searchFilter);
			} catch (Exception e) {
				e.printStackTrace();
			} 
		 }
	...
	public String gmbAdd() {
		try{
			...
			//进行保存（单个/批量）
				...
				//跳转到指定页面
				this.redirectUrl = "Gmbbaseuser!gmbList.action";
				return SUCCESS;
			}
		} catch (Exception e){
			e.printStackTrace();
	...
	private String ajaxList(){
		if(!exportState){
		  ...
		  WriteJsonArray(page.getResult());
			...
			//获取查询数据
			gmbbaseuserList = getList().getResult();
			//导出Excel
			fileName = ExcelUtil.exportExcel(fields, gmbbaseuserList, getRequest().getRealPath("/excelTemp"));
			return "exportExcel";
		}
	    return "none";
	}
	 ...
	 */
	 private int getSave(){
	 	try {
			if(gmbbaseuser!=null){
		    	gmbbaseuser.setId(StringUtil.getUUID());
				gmbbaseuser.setPassword(StringUtil.md5(gmbbaseuser.getPassword()));
				gmbbaseuserService.save(gmbbaseuser);
			}else{
				for(Gmbbaseuser gmbbaseuser:gmbbaseuserList){
					gmbbaseuser.setId(StringUtil.getUUID());
					gmbbaseuser.setPassword(StringUtil.md5(gmbbaseuser.getPassword()));
				}
				gmbbaseuserService.save(gmbbaseuserList);
			}
			//执行成功返回1
			return 1;
		} catch (Exception e) {
		...
		if(!importState){
			...
			Hashtable<String, Object> msg = new Hashtable<String, Object>();
			 ...
			 //判断内容是否为空
				 ...
				 try {
					gmbbaseuserService.save(excelDynamicList);
					msg.put("state", true);
					msg.put("msg", "保存成功！");
	...
	public String gmbUpdate() {
		try {
			...
			if(StringUtil.isNotEmpty(handle) && (handle.equalsIgnoreCase("forwardUpdatePage"))){
				try {
					gmbbaseuser = gmbbaseuserService.get(id);
				} catch (Exception e) {
					e.printStackTrace();
				}
				return INPUT;
	...
	private String ajaxInfo(){
		...
		//返回相应页码
		return WriteJSONObject(gmbbaseuser);
	}
	...
	private int getUpdate(){
		try {
			...
			if(!this.gmbbaseuser.getPassword().equalsIgnoreCase(gmbbaseuser.getPassword())){
				this.gmbbaseuser.setPassword(StringUtil.md5(this.gmbbaseuser.getPassword()));
			}
			BeanUtils.copyProperties(this.gmbbaseuser, gmbbaseuser, new String[] {"id"});
	...
	public String uniqueValid() {
		...
		try {
			...
			writeJSON = writeJSON(gmbbaseuserService.isExist(searchFilter));
		} catch (Exception e) {
			e.printStackTrace();
		}
		return writeJSON;
	...
	public Gmbbaseuser getGmbbaseuser() {
		return gmbbaseuser;
	}
	
	public void setGmbbaseuser(Gmbbaseuser gmbbaseuser) {
		this.gmbbaseuser = gmbbaseuser;
	}
	
	public List<Gmbbaseuser> getGmbbaseuserList(){
		return gmbbaseuserList;
	}
	
	public void setGmbbaseuserList(List<Gmbbaseuser> gmbbaseuserList){
		this.gmbbaseuserList=gmbbaseuserList;
	}
